...
class NewsListController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var articleCollection : ArticleCollection?
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
       // return newsCollection?.posts?.count ?? 0
        return articleCollection?.articles?.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell  : NewsCell? = tableView.dequeueReusableCell(withIdentifier: "NewsCell", for: indexPath) as? NewsCell
        //cell?.news = newsCollection?.posts?[indexPath.row]
        cell?.article = articleCollection?.articles?[indexPath.row]
        return cell!
    }
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       // let news = newsCollection?.posts?[indexPath.row]
       // self.performSegue(withIdentifier: "goToDetail", sender: news)
        let article = self.articleCollection?.articles?[indexPath.row]
        self.performSegue(withIdentifier: "goToDetail", sender: article)
    }

    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "goToDetail" {
            let detailController = segue.destination as? DetailController
            //detailController?.news = sender as? News
            detailController?.article = sender as? Article
        }
    }
    ...
    func fetchNews()  {
        ...
        let task = URLSession.shared.dataTask(with: url) { (data, response, error) in
            ...
            do{
                ...
                let jsonDecoder = JSONDecoder()
                print(jsonResponse) //Response result
                self.articleCollection  = try jsonDecoder.decode(ArticleCollection.self, from: dataResponse)
